---
type: game
title: "Digimon World 3"
platform: "PS1"
genre: "JRPG / Monster Collector"
status: playing
completion: 0
playtime: 0
chapter: "ชนะ Byakko Leader (Badge 3/4) · Partner ครบ 8/8"
started_date: 
completed_date: 
rating: 
version: "US (Versus Books Official Perfect Guide)"
tags: [game, digimon, ps1]
---

# Digimon World 3

> [!abstract] เปิดมาอ่านตรงนี้ก่อน
> **เนื้อเรื่องถึง:** ชนะ Byakko Leader แล้ว (Badge ใบที่ 3) → ต่อไป Genbu Leader
> **ทำต่อ:** ตาม [[แผนการฝึก]] — Agumon→Diaboromon · Veemon→Beelzemon+Imperialdramon-PM · Kotemon→Seraphimon+Phoenixmon
> **✅ Partner ครบ 8/8** — ไม่มีเส้นตายเลเวลอีกแล้ว ฟาร์มได้เต็มที่
> **เซฟไว้ที่:** 

## ความคืบหน้า

| หมวด | เก็บได้ | ทั้งหมด | % |
|---|---|---|---|
| Sector Badge | 3 | 4 | 75% |
| Partner Digimon | **8** | 8 | **100%** ✅ |
| เควส DNA | ✅ | — | **เก็บครบแล้ว ไม่มี missable เหลือ** |
| Digivolution | ? | ~350 | ? |
| การ์ด (Card Battle) | ? | ? | ? |
| **รวม** | | | **?%** |

---

## 📖 เนื้อเรื่อง

- [[00-เรื่องย่อ]] · [[01-สรุปรายบท]] · [[02-ตัวละคร]]
- [[03-โลกและไทม์ไลน์]] · [[04-ตอนจบ]] · [[05-ปมที่ยังไม่เฉลย]]

## ✅ เก็บให้ครบ

- [[02-Roadmap]] — ลำดับ Leader ทั้ง 4
- [[Missables]] — ⚠️ **เควส DNA มีเส้นตายเป็นเลเวล อ่านด่วน**
- [[Digivolution]] · [[Items-Equipment]] · [[Enemy-Digimon]] — ข้อมูลจาก guide
- [[Partners]] — Partner Digimon ทั้ง 8 ตัว
- [[Key-Items]] — 🗝️ ของสำคัญทั้งเกม ใครถือ อยู่ที่ไหน
- [[Collectibles]] · [[Bosses]] · [[Quests]] · [[Achievements]]

## 📂 อื่นๆ

- `04-Areas/` — โซนตามแผนที่
- [[แผนการฝึก]] — 🎯 เส้นทางสู่ร่างขั้นสุด
- [[แผนอัพสเตตัส]] — 📊 ใครขาดสเตตัสอะไรกี่แต้ม
- [[Digivolution-Slot]] — 🎰 ควรพกร่างไหนไว้ 3 ช่อง
- [[Veemon-ลำดับปั้น]] — 🐉 ลำดับปั้น 5 ร่างของ Veemon
- [[Training-Gym]] — 🏋️ ฝึกสเตตัสด้วย TP
- `05-Builds/` — ทีมและอุปกรณ์
- `06-Sessions/` — บันทึกรายวัน
- [[99-Sources]]

---

## กติกาที่ตั้งไว้เอง

- [ ] เก็บ Partner ให้ครบทั้ง 8 ตัว (รวม Veemon)
- [ ] เช็คเลเวลรวมก่อนฟาร์มทุกครั้ง — เลเวลเกินแล้วเควสหาย
