---
type: reference
subtype: digivolution
game: "Digimon World 3"
partner: Veemon
---

# 🐉 ผัง Digivolution — Veemon

> guide หน้า 75 · **สายถนัด: ExVeemon → Paildramon → Imperialdramon**
> ⚠️ Veemon เป็น Partner ตัวเดียวที่ได้จาก **Card Battle** ไม่ใช่เควส DNA

## สายมังกร (สายประจำตัว)

```
Veemon
 └─ ExVeemon ................. Veemon 5
     └─ Paildramon ........... Veemon 20 + Stingmon 5
         └─ Imperialdramon ... Veemon 40
             ├─ Imperialdramon-F ... Imperialdramon 99
             └─ Imperialdramon-P ... Imperialdramon-F 40 + Omnimon 40
```

| ร่าง | เงื่อนไข |
|---|---|
| **ExVeemon** | **Veemon 5** ⬅ ถูกที่สุดในเกม |
| **Paildramon** ⭐ | **Veemon 20** **+** Stingmon 5 |
| **Imperialdramon** | **Veemon 40** |
| **Imperialdramon-F** | Imperialdramon 99 |
| **Imperialdramon-P** ⭐ | Imperialdramon-F 40 **+** Omnimon 40 |

> ⭐ **Veemon เข้าถึงสายมังกรถูกที่สุดในบรรดา Partner ทั้งหมด** — ตัวอื่นต้องอ้อมผ่าน Greymon หรือ Taomon ก่อน

## สายแมลง

| ร่าง | เงื่อนไข |
|---|---|
| **Stingmon** | ExVeemon 30 |
| **Kabuterimon** | Stingmon 40 |
| **GranKuwagamon** | Imperialdramon 40 **+** Kabuterimon 40 |
| **Diaboromon** ⭐ | GranKuwagamon 99 **+** BlackWarGreymon 99 |

## สาย Greymon

| ร่าง | เงื่อนไข |
|---|---|
| **Greymon** | Paildramon 35 **+ Defense 200** |
| **MetalGreymon** | Greymon 40 |
| **WarGreymon** | MetalGreymon 99 |
| **SkullGreymon** | Greymon 50 **+ Dark 190** |
| **BlackWarGreymon** | SkullGreymon 99 |
| **Omnimon** ⭐ | WarGreymon 40 **+** MetalGarurumon 40 |

## สาย Machine

| ร่าง | เงื่อนไข |
|---|---|
| **MetalMamemon** | Paildramon 20 **+ Machine 100** |
| **MetalGarurumon** | MetalMamemon 40 **+ Machine 140** |
| **MegaGargomon** | MetalGarurumon 99 |
| **Hookmon** | Imperialdramon 35 **+ Machine** |
| **Armormon** | Hookmon 50 |
| **Cannondramon** | Armormon 99 |

## สายอัศวิน / สัตว์

| ร่าง | เงื่อนไข |
|---|---|
| **Growlmon** | Paildramon 30 **+ Fire 90** |
| **WarGrowlmon** | Growlmon 50 |
| **Gallantmon** | WarGrowlmon 99 |
| **Grizzmon** | Imperialdramon 45 **+ Defense** |
| **GrapLeomon** | Grizzmon 50 |
| **Marsmon** | GrapLeomon 99 |

## สายเวท

| ร่าง | เงื่อนไข |
|---|---|
| **Kyubimon** | Paildramon 40 **+ Ice 200** |
| **Taomon** | Kyubimon 50 |
| **Sakuyamon** | Taomon 99 |
| **Digitamamon** | Imperialdramon 20 **+ Intelligence** |

## สายเทวดา

| ร่าง | เงื่อนไข |
|---|---|
| **Angemon** | Paildramon 45 **+ Intelligence 230** |
| **MagnaAngemon** | Angemon 50 |
| **Seraphimon** | MagnaAngemon 99 |
| **Angewomon** | MagnaAngemon 20 |
| **Rosemon** | Angewomon 40 |
| **Phoenixmon** | Rosemon 99 |

## สายซามูไร / มืด

| ร่าง | เงื่อนไข |
|---|---|
| **Dinohumon** | (ผ่าน Imperialdramon) |
| **Kyukimon** | Dinohumon 50 |
| **GuardiAngemon** | Kyukimon 99 |
| **Devimon** | Paildramon 25 **+ Dark 160** |
| **Myotismon** | Devimon 40 |
| **MaloMyotismon** | Myotismon 99 |
| **Beelzemon** ⭐ | MaloMyotismon 40 **+** Gallantmon 40 |

---

## 🎯 แผนของ Veemon ตอนนี้ (LV15)

ปลดแล้ว: ยังไม่ปลด **ExVeemon** (ต้อง Veemon 5 — ผ่านเลเวลแล้ว ลองเช็คในเกม)

| เป้าหมายถัดไป | ต้องทำอะไร |
|---|---|
| **ExVeemon** | Veemon LV5 — **น่าจะปลดได้แล้ว** ไปกดดูในเมนู Digivolve |
| **Paildramon** | Veemon **LV20** (อีก 5 เลเวล) + ต้องมี **Stingmon LV5** ก่อน |
| **Stingmon** | ExVeemon LV30 |
| **Imperialdramon** | Veemon **LV40** |

> 💡 **Veemon คือทางที่ถูกที่สุดไปสาย Imperialdramon** — คุ้มที่จะปั้นต่อ แต่ ⚠️ ทุกเลเวลที่เพิ่มดันเลเวลรวมเข้าใกล้ 90 → เคลียร์เควส DNA ที่ Mobius Desert ก่อน ([[Missables]])
